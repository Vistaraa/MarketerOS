    ...
    >
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-zinc-200/90 bg-white shadow-sm overflow-hidden dark:border-zinc-800 dark:bg-zinc-950/60">
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-zinc-100 bg-zinc-50/50 text-[10px] font-semibold uppercase tracking-wider text-zinc-400 dark:border-zinc-800 dark:bg-zinc-900/40">
                  <th className="py-3 pl-5 pr-3">Platform</th>
                ...
                {DEFAULT_PLATFORMS_TABLE.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Daily Budget */}
                      <td className="px-3 py-3.5 text-zinc-900 dark:text-zinc-100">
                        ${item.budget.toLocaleString()}{" "}
                        <span className="text-[10px] text-zinc-400">/day</span>
                      </td>

                      {/* Spend */}
                      <td className="px-3 py-3.5 font-semibold text-zinc-900 dark:text-zinc-100">
                        ${item.spend.toLocaleString()}
                      </td>

                      {/* Clicks */}
                      <td className="px-3 py-3.5 text-zinc-600 dark:text-zinc-400">
                      ...
                      <td className="py-3.5 pl-3 pr-5 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1.5 text-zinc-400">
                          ...
                          <button
                            ...
                            className="grid h-7 w-7 place-items-center rounded-lg border border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
                            title="Edit"
                          >
                            <Edit3 size={13} />
                          </button>
                          <button
                            className="grid h-7 w-7 place-items-center rounded-lg text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100"
                            title="More"
                          >
          ...
          {/* Table Pagination */}
          <div className="flex items-center justify-end gap-1 border-t border-zinc-100 p-3 text-xs dark:border-zinc-800">
            <button className="grid h-6 w-6 place-items-center rounded border border-zinc-200 text-zinc-400 hover:bg-zinc-50 dark:border-zinc-800">
              <ChevronLeft size={13} />
            </button>
            {[1, 2, 3, 4].map((page) => (
              <button
                ...
                className={cn(
                  "grid h-6 w-6 place-items-center rounded text-xs font-semibold transition",
                  currentPage === page
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12 items-start">
          ...
          <div className="lg:col-span-4 rounded-xl border border-zinc-200/90 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/60 flex flex-col justify-between">
            <div>
              {/* Header */}
              <div className="flex items-center gap-3">
                <PlatformIcon platform={selectedPlatform.platform} size={32} />
                <div>
                  <h2 className="text-base font-bold text-zinc-900 dark:text-zinc-100">
                    {selectedPlatform.name}
                  </h2>
                  <div className="text-xs text-zinc-400">{selectedPlatform.type}</div>
                </div>
              </div>

              {/* 4x2 Detail Attributes Grid */}
              <div className="mt-6 grid grid-cols-2 gap-y-4 gap-x-6 border-t border-zinc-100 pt-5 text-xs dark:border-zinc-800">
                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Budget</span>
                ...
                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Start Date</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100">{selectedPlatform.startDate}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">End Date</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100">{selectedPlatform.endDate}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Objective</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100">{selectedPlatform.objective}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Target ROAS</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100">{selectedPlatform.targetRoas.toFixed(1)}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Bidding Strategy</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100 truncate">{selectedPlatform.biddingStrategy}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Location</span>
                  <strong className="mt-0.5 block text-xs text-zinc-900 dark:text-zinc-100">{selectedPlatform.location}</strong>
                </div>

                <div>
                  <span className="text-[10px] text-zinc-400 uppercase tracking-wider font-semibold">Devices</span>
              ...
              <div className="mt-6 border-t border-zinc-100 pt-5 text-xs dark:border-zinc-800">
          ...
          <div className="lg:col-span-8 rounded-xl border border-zinc-200/90 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/60">
            {/* Top 5 KPI Tiles */}
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5 border-b border-zinc-100 pb-5 dark:border-zinc-800">
              <div>
                <span className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">Spend</span>
                <div className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">${selectedPlatform.spend.toLocaleString()}</div>
                <div className="mt-0.5 flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
                  <ArrowUpRight size={11} /> 8.4%
                </div>
              </div>

              <div>
                <span className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">Clicks</span>
                <div className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">{selectedPlatform.clicks.toLocaleString()}</div>
                <div className="mt-0.5 flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
                  <ArrowUpRight size={11} /> 12.6%
                </div>
              </div>

              <div>
                <span className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">Conversions</span>
                <div className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">{selectedPlatform.conversions.toLocaleString()}</div>
                <div className="mt-0.5 flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
                  <ArrowUpRight size={11} /> 18.3%
                </div>
              </div>

              <div>
                <span className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">Cost / Conv.</span>
                <div className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">${selectedPlatform.cpa.toFixed(2)}</div>
                <div className="mt-0.5 flex items-center gap-0.5 text-[10px] font-semibold text-rose-600 dark:text-rose-400">
                  <ArrowDownRight size={11} /> 3.2%
                </div>
              </div>

              <div>
                <span className="text-[11px] text-zinc-500 dark:text-zinc-400 font-medium">ROAS</span>
                <div className="mt-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">{selectedPlatform.roas.toFixed(2)}</div>
                <div className="mt-0.5 flex items-center gap-0.5 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400">
                  <ArrowUpRight size={11} /> 16.7%
                </div>
              </div>
            </div>
            ...
            <div className="mt-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <select
                    ...
                    className="h-6 rounded border border-zinc-200 bg-white px-2 text-[11px] font-medium text-zinc-700 outline-none dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-300"
                  >
                    <option>Daily</option>
              ...
              <div className="mt-4 h-60 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={PLATFORM_DAILY_SERIES} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
