...
export async function POST(request: Request, { params }: { params: { path: string[] } }) {
  ...
  if (path === "logout") { await clearSession(); return NextResponse.json({ data: { success: true } }); }
  if (path === "signup") {
    ...
    await setSession({ userId: result.user.id, workspaceId: result.workspace.id, role: "OWNER", email: result.user.email, name: `${result.user.firstName} ${result.user.lastName}` });
    return NextResponse.json({ data: { success: true, next: "/onboarding/brand" } }, { status: 201 });
  }
  if (path === "login") {
    ...
    return NextResponse.json({ data: { success: true, next: "/overview" } });
  }
